...
function buildYearlyData(rows) {
  ...
  return Array.from(grouped.values())
    ...
    .map((item) => {
      ...
      const neighborNetRevenue = Math.max(neighborGrossRevenue - neighborServiceFee, 0);
...
export default function App() {
  ...
  const totals = useMemo(() => {
    const base = yearlyData.reduce(
      ...
      { productionKwh: 0, selfConsumedKwh: 0, grossExportedKwh: 0, neighborExportKwh: 0, exportedKwh: 0, gridPurchaseKwh: 0, ownUseSavings: 0, feedInRevenue: 0, neighborGrossRevenue: 0, neighborServiceFee: 0, neighborNetRevenue: 0, totalFeedInRevenue: 0, annualBenefit: 0 }
    );
            ...
            <tbody>
              {yearlyData.map((row) => (
                <tr key={row.year} className={row.monthsWithValues === 0 ? 'empty-row' : ''}>
                  ...
                  <td>{row.neighborServiceFee > 0 ? currency.format(row.neighborServiceFee) : '–'}</td>
